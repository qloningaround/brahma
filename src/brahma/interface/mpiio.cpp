...
GOTCHA_MACRO(PMPI_File_set_size, int, (MPI_File fh, MPI_Offset size),
...
GOTCHA_MACRO(PMPI_File_iread, int,
...
GOTCHA_MACRO(PMPI_File_read, int,
             (MPI_File fh, void *buf, int count, MPI_Datatype datatype,
              MPI_Status *status),
             (fh, buf, count, datatype, status), brahma::MPIIO);

GOTCHA_MACRO(PMPI_File_read_ordered_begin, int,
             (MPI_File fh, void *buf, int count, MPI_Datatype datatype),
             (fh, buf, count, datatype), brahma::MPIIO);
...
GOTCHA_MACRO(PMPI_File_write_all_begin, int,
             (MPI_File fh, const void *buf, int count, MPI_Datatype datatype),
             (fh, buf, count, datatype), brahma::MPIIO);
...
GOTCHA_MACRO(PMPI_File_write_ordered_begin, int,
             (MPI_File fh, const void *buf, int count, MPI_Datatype datatype),
             (fh, buf, count, datatype), brahma::MPIIO);
...
int MPIIO::PMPI_File_iread_at(MPI_File fh, MPI_Offset offset, void *buf,
                              int count, MPI_Datatype datatype,
                              MPI_Request *request) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_iread_at, int,
                        (fh, offset, buf, count, datatype, request));
}

int MPIIO::PMPI_File_iread(MPI_File fh, void *buf, int count,
                           MPI_Datatype datatype, MPI_Request *request) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_iread, int,
                        (fh, buf, count, datatype, request));
}

int MPIIO::PMPI_File_iread_shared(MPI_File fh, void *buf, int count,
                                  MPI_Datatype datatype, MPI_Request *request) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_iread_shared, int,
                        (fh, buf, count, datatype, request));
}

int MPIIO::PMPI_File_iwrite_at(MPI_File fh, MPI_Offset offset, const void *buf,
                               int count, MPI_Datatype datatype,
                               MPI_Request *request) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_iwrite_at, int,
                        (fh, offset, buf, count, datatype, request));
}

int MPIIO::PMPI_File_iwrite(MPI_File fh, const void *buf, int count,
                            MPI_Datatype datatype, MPI_Request *request) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_iwrite, int,
                        (fh, buf, count, datatype, request));
}

int MPIIO::PMPI_File_iwrite_shared(MPI_File fh, const void *buf, int count,
                                   MPI_Datatype datatype,
                                   MPI_Request *request) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_iwrite_shared, int,
                        (fh, buf, count, datatype, request));
}

int MPIIO::PMPI_File_open(MPI_Comm comm, const char *filename, int amode,
                          MPI_Info info, MPI_File *fh) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_open, int, (comm, filename, amode, info, fh));
}

int MPIIO::PMPI_File_read_all_begin(MPI_File fh, void *buf, int count,
                                    MPI_Datatype datatype) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_read_all_begin, int,
                        (fh, buf, count, datatype));
}

int MPIIO::PMPI_File_read_all(MPI_File fh, void *buf, int count,
                              MPI_Datatype datatype, MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_read_all, int,
                        (fh, buf, count, datatype, status));
}

int MPIIO::PMPI_File_read_at_all(MPI_File fh, MPI_Offset offset, void *buf,
                                 int count, MPI_Datatype datatype,
                                 MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_read_at_all, int,
                        (fh, offset, buf, count, datatype, status));
}

int MPIIO::PMPI_File_read_at_all_begin(MPI_File fh, MPI_Offset offset,
                                       void *buf, int count,
                                       MPI_Datatype datatype) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_read_at_all_begin, int,
                        (fh, offset, buf, count, datatype));
}

int MPIIO::PMPI_File_read_at(MPI_File fh, MPI_Offset offset, void *buf,
                             int count, MPI_Datatype datatype,
                             MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_read_at, int,
                        (fh, offset, buf, count, datatype, status));
}

int MPIIO::PMPI_File_read(MPI_File fh, void *buf, int count,
                          MPI_Datatype datatype, MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_read, int,
                        (fh, buf, count, datatype, status));
}

int MPIIO::PMPI_File_read_ordered_begin(MPI_File fh, void *buf, int count,
                                        MPI_Datatype datatype) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_read_ordered_begin, int,
                        (fh, buf, count, datatype));
}

int MPIIO::PMPI_File_read_ordered(MPI_File fh, void *buf, int count,
                                  MPI_Datatype datatype, MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_read_ordered, int,
                        (fh, buf, count, datatype, status));
}

int MPIIO::PMPI_File_read_shared(MPI_File fh, void *buf, int count,
                                 MPI_Datatype datatype, MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_read_shared, int,
                        (fh, buf, count, datatype, status));
}
...
int MPIIO::PMPI_File_write_all_begin(MPI_File fh, const void *buf, int count,
                                     MPI_Datatype datatype) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_write_all_begin, int,
                        (fh, buf, count, datatype));
}

int MPIIO::PMPI_File_write_all(MPI_File fh, const void *buf, int count,
                               MPI_Datatype datatype, MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_write_all, int,
                        (fh, buf, count, datatype, status));
}

int MPIIO::PMPI_File_write_at_all_begin(MPI_File fh, MPI_Offset offset,
                                        const void *buf, int count,
                                        MPI_Datatype datatype) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_write_at_all_begin, int,
                        (fh, offset, buf, count, datatype));
}

int MPIIO::PMPI_File_write_at_all(MPI_File fh, MPI_Offset offset,
                                  const void *buf, int count,
                                  MPI_Datatype datatype, MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_write_at_all, int,
                        (fh, offset, buf, count, datatype, status));
}

int MPIIO::PMPI_File_write_at(MPI_File fh, MPI_Offset offset, const void *buf,
                              int count, MPI_Datatype datatype,
                              MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_write_at, int,
                        (fh, offset, buf, count, datatype, status));
}

int MPIIO::PMPI_File_write(MPI_File fh, const void *buf, int count,
                           MPI_Datatype datatype, MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_write, int,
                        (fh, buf, count, datatype, status));
}

int MPIIO::PMPI_File_write_ordered_begin(MPI_File fh, const void *buf,
                                         int count, MPI_Datatype datatype) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_write_ordered_begin, int,
                        (fh, buf, count, datatype));
}

int MPIIO::PMPI_File_write_ordered(MPI_File fh, const void *buf, int count,
                                   MPI_Datatype datatype, MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_write_ordered, int,
                        (fh, buf, count, datatype, status));
}

int MPIIO::PMPI_File_write_shared(MPI_File fh, const void *buf, int count,
                                  MPI_Datatype datatype, MPI_Status *status) {
  BRAHMA_UNWRAPPED_FUNC(PMPI_File_write_shared, int,
                        (fh, buf, count, datatype, status));
}
}  // namespace brahma